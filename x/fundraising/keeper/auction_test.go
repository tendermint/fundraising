...
func (s *KeeperTestSuite) TestAuctionStatus() {
	standByAuction := s.createFixedPriceAuction(
		...
		"denom4",
		[]types.VestingSchedule{},
		types.MustParseRFC3339("2010-01-01T00:00:00Z"),
		types.MustParseRFC3339("2010-01-10T00:00:00Z"),
		true,
	)
...
func (s *KeeperTestSuite) TestDistributeSellingCoin() {
	...
	auction := s.createFixedPriceAuction(
		...
		types.MustParseRFC3339("2022-01-21T00:00:00Z"),
		true,
	)
	...
	s.Require().True(s.getBalance(bidder1, auction.GetSellingCoin().Denom).IsPositive())
	s.Require().True(s.getBalance(bidder2, auction.GetSellingCoin().Denom).IsPositive())
	s.Require().True(s.getBalance(bidder3, auction.GetSellingCoin().Denom).IsPositive())
}
...
func (s *KeeperTestSuite) TestDistributePayingCoin() {
	...
	s.placeBid(auction.Id, s.addr(1), sdk.OneDec(), sdk.NewInt64Coin(auction.GetPayingCoinDenom(), 100_000_000), true)
	s.placeBid(auction.Id, s.addr(1), sdk.OneDec(), sdk.NewInt64Coin(auction.GetPayingCoinDenom(), 200_000_000), true)
	s.placeBid(auction.Id, s.addr(1), sdk.OneDec(), sdk.NewInt64Coin(auction.GetPayingCoinDenom(), 200_000_000), true)
	...
	_, found = s.keeper.GetAuction(s.ctx, auction.GetId())
	s.Require().True(found)
	s.Require().Equal(types.AuctionStatusFinished, auction.GetStatus())
}

func (s *KeeperTestSuite) TestCancelAuction() {
	standByAuction := s.sampleFixedPriceAuctions[0]

	s.SetAuction(standByAuction)

	auction, found := s.keeper.GetAuction(s.ctx, standByAuction.GetId())
	s.Require().True(found)
	s.Require().Equal(types.AuctionStatusStandBy, auction.GetStatus())

	s.CancelAuction(auction)

	auction, found = s.keeper.GetAuction(s.ctx, standByAuction.GetId())
	s.Require().True(found)
	s.Require().Equal(types.AuctionStatusCancelled, auction.GetStatus())

	sellingReserve := s.app.BankKeeper.GetBalance(s.ctx, auction.GetSellingReserveAddress(), auction.GetSellingCoin().Denom)
	s.Require().Equal(sdk.NewCoin(auction.GetSellingCoin().Denom, sdk.ZeroInt()), sellingReserve)
}
